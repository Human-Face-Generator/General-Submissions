...
class Pix2PixHDModel(BaseModel):
    ...
    def encode_input(self, inter_label_map_1, label_map, inter_label_map_2, real_image, label_map_ref, real_image_ref, infer=False):
        ...
        if self.opt.label_nc == 0:
            ...
            inter_label_1 = inter_label_map_1.data.to(device)
            inter_label_2 = inter_label_map_2.data.to(device) 
            input_label_ref = label_map_ref.data.to(device)
        else:
            ...
            inter_label_1 = inter_label_1.scatter_(1, inter_label_map_1.data.long().to(device), 1.0)
            inter_label_2 = torch.FloatTensor(torch.Size(oneHot_size)).zero_()
            inter_label_2 = inter_label_2.scatter_(1, inter_label_map_2.data.long().to(device), 1.0)
            input_label_ref = torch.FloatTensor(torch.Size(oneHot_size)).zero_()
            input_label_ref = input_label_ref.scatter_(1, label_map_ref.data.long().to(device), 1.0) 
        ...
        real_image_ref = Variable(real_image_ref.data.to(device))
